"""
TCP Reno-style UDP sender.
Initial cwnd = 1 packet, ssthresh = 64 packets.
Slow start: cwnd += 1 per ACK until cwnd >= ssthresh.
Congestion avoidance: cwnd += 1/cwnd per ACK.
Timeout: ssthresh = cwnd/2, cwnd = 1.
Triple duplicate ACK: ssthresh = cwnd/2, cwnd = ssthresh, fast retransmit.
"""
import socket
import time
...
# Longer timeout: training profile has ~100ms delay (RTT ~200ms) + loss; 0.5s caused frequent timeouts and cwnd=1 resets
TIMEOUT_S = 1.2
...
def run_reno(file_bytes, host, port):
    packet_delays = []
    first_send_times = {}
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        ...
        send_base = 0
        next_seq = 0
        cwnd_packets = float(INITIAL_CWND_PACKETS)
        ssthresh_packets = float(INITIAL_SSTHRESH_PACKETS)
        last_ack_seq = -1
        duplicate_ack_count = 0
        transfer_end = None

        while send_base < file_len:
            cwnd_bytes = int(cwnd_packets) * MESSAGE_SIZE

            # Send up to cwnd packets in flight
            while next_seq < file_len and (next_seq - send_base) < cwnd_bytes:
            ...
            try:
                # Process one or more ACKs (drain pending ACKs so cwnd grows faster per RTT)
                    ...
                    if ack_msg == b"ack":
                        if ack_seq == last_ack_seq:
                            ...
                            if duplicate_ack_count == DUPLICATE_ACK_THRESHOLD:
                                # Fast retransmit
                                ssthresh_packets = max(1.0, int(cwnd_packets) // 2)
                                cwnd_packets = ssthresh_packets
                                if send_base < file_len:
                                    chunk = file_bytes[send_base : send_base + MESSAGE_SIZE]
                                    sock.sendto(make_packet(send_base, chunk), (host, port))
                        else:
                            duplicate_ack_count = 0
                            ...
                            while pos < new_send_base and pos < file_len:
                                if pos in first_send_times:
                                    packet_delays.append(
                                        time.perf_counter() - first_send_times[pos]
                                    )
                            ...
                            if cwnd_packets < ssthresh_packets:
                                cwnd_packets += 1
                            else:
                                cwnd_packets += 1.0 / max(1, int(cwnd_packets))

                    # Try to read another ACK without blocking (grow cwnd faster)
                    sock.settimeout(0.0)
                    try:
                        ack_packet, _ = sock.recvfrom(PACKET_SIZE)
    ...
    avg_delay = (
        sum(packet_delays) / len(packet_delays) if packet_delays else 0.0
    )
    metric = (
        0.3 * (throughput / 1000.0) + 0.7 * (1.0 / avg_delay)
        if avg_delay
        else 0.0
    )
...
def main():
    ...
    throughput, avg_delay, metric = run_reno(
